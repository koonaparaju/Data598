...
file_suffixes_to_evaluate=c(file_suffix, "ets")
...
force_write_new_train_test=TRUE
...
convert_to_xts=function(df, clean=TRUE){
  ...
  return(xts(df[ ,c("sid", "Open", "High", "Low", "Volume", "Close")], order.by=df$Date, unique=FALSE, frequency = 7))
}
...
apply_to_each_sid = function(xts_stocks, f, stock_id_col="sid", debug=FALSE, failOnError=failOnError) {
  sids = sort(unique(drop(coredata(xts_stocks[,stock_id_col]))))
  ret_val=list()
  for(s in sids){
    if(debug){
...
get_sample_paths_for_all_stocks=function(xts_stocks, model, simulation_func=function(x,h) {simulate_to_vec(x,h)}, number_of_sample_paths=500, horizon_list=NULL, plotForecasts=FALSE, plotFunc=NULL, debug=FALSE, final_submission=FALSE, failOnError=failOnError) {
...
get_random_walk = function(xts_stocks, sid, forecast_to=as.Date("2020-01-31")) {
  ...
  len_test=length(get_sid_data(test,sid)[,"sid"])
  if(len_test != horizon)
  {
...
train_after_transformation=apply_to_each_sid_mod(train, function(xts_stocks, sid) {return(get_lag_col(xts_stocks, sid, "Open", 1))})
...
get_random_walk = function(xts_stocks, sid, forecast_to=as.Date("2020-01-31")) {
  ...
  return(cumsum(sample(log(rt(1000, df=length(xts_stocks[,"sid"])-1)), horizon, replace=TRUE)))
}
...
evaluate_per_stock_id=function(df, s, test_df=test) {
    return(evaluate(as.numeric(test_df[test_df[,"sid"]==s,"Close"]), as.matrix(df[,!names(df) %in% c("stock_id", "Date")])))
}
