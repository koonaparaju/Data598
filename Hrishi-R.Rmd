...
model_to_run=function(train_data){
  ...
  fitted_model=auto.arima(coredata(drop(train_data[,"Close"])))#, xreg=coredata(train_data[,"Open.1"]))
  return(fitted_model)
}
...
apply_to_each_sid_mod = function(xts_stocks, f, stock_id_col="sid", debug=FALSE, failOnError=shouldFailOnError) {
...
apply_to_each_sid = function(xts_stocks, f, stock_id_col="sid", debug=FALSE, failOnError=shouldFailOnError) {
  sids = sort(unique(drop(coredata(xts_stocks[,stock_id_col]))))
  ret_val=list()
  for(s in sids){
    if(debug){
